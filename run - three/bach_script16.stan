...
parameters { // raw parameters to be adjusted
  ...
  real<lower=0.0, upper=1.0> medb1raw;
  real<lower=0.0, upper=1.0> medb2raw;
  real<lower=0.0, upper=1.0> medd1raw;       
  real<lower=0.0, upper=1.0> meda2raw;       
  real<lower=0.0, upper=1.0> slowb0raw;
  real<lower=0.0, upper=1.0> slowb1raw;
  real<lower=0.0, upper=1.0> slowb2raw;
  real<lower=0.0, upper=1.0> slowd1raw;       
  real<lower=0.0, upper=1.0> slowa2raw;       
...
transformed parameters { // additional parameters to be used by the model
  ...
  // scale and satisfy Su et al 2016 WRR equation 33
  ...
  // Eckhardt parameters FIXME
  medBFImax = medb0/(1-meda1);
  medrec = medb0<1 ? meda1/(1-medb0) : 0;
  
  // split a1 and b0
  meda2 = meda1*meda2raw;
  meda1 = meda1*(1-meda2raw);
  medb2 = medb0*medb1raw*medb2raw;
  medb1 = medb0*medb1raw*(1-medb2raw);
  medb0 = medb0*(1-medb1raw);
  ...
  // split a1 and b0
  slowa2 = slowa1*slowa2raw;
  slowa1 = slowa1*(1-slowa2raw);
  slowb2 = slowb0*slowb1raw*slowb2raw;
  slowb1 = slowb0*slowb1raw*(1-slowb2raw);
  slowb0 = slowb0*(1-slowb1raw);
  ...
  // initial conditions (to allow for GENth(2,2) filters)
  ...
  for (n in (startrun+2):endvalid) {
    ...
    bach[n,2] = meda1*bach[n-1,2] + meda2*bach[n-2,2] + medb0*bach[n,1] + medb1*bach[n-1,1] + medb2*bach[n-2,1];
    if (bach[n,2] > bach[n,1]) 
      bach[n,2] = bach[n,1];
    // bach[n,3] = slowa1*bach[n-1,3] + slowb0*bach[n,2];
    bach[n,3] = slowa1*bach[n-1,3] + slowa2*bach[n-2,3] + slowb0*bach[n,2] + slowb1*bach[n-1,2] + slowb2*bach[n-2,2];
...
model { // compare model with data
  ...
  medb1raw ~ normal(0.5, priorwide);
  medb2raw ~ normal(0.5, priorwide);
  medd1raw ~ normal(0.4, priorwide); 
  meda2raw ~ normal(0.0, priornarrow); 
  slowb0raw ~ normal(1.0, priornarrow);
  slowb1raw ~ normal(0.5, priorwide);
  slowb2raw ~ normal(0.5, priorwide);
  slowd1raw ~ normal(0.7, priorwide); 
  slowa2raw ~ normal(0.0, priornarrow); 
